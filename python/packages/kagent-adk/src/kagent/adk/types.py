...
def create_header_provider(
    ...
    base_header_provider: Callable[[Optional[ReadonlyContext]], dict[str, str]] | None = None,
    ...
    Args:
        ...
        base_header_provider: Optional base header provider whose headers are applied
                             first (lowest precedence). Used for dynamically reading
                             auth tokens from volume-mounted Kubernetes Secrets.

    Returns:
        A header provider function, or None if no headers need to be propagated.
    """
    if not allowed_headers and not sts_header_provider and not base_header_provider:
        return None
    ...
    def header_provider(readonly_context: Optional[ReadonlyContext]) -> dict[str, str]:
        ...
        # 0. Apply base headers first (lowest precedence).  This is used for
        # dynamically reading rotating auth tokens from mounted Secret files.
        if base_header_provider:
            base_headers = base_header_provider(readonly_context)
            if base_headers:
...
def create_file_header_provider(
    header_files: dict[str, str],
) -> Callable[[Optional[ReadonlyContext]], dict[str, str]]:
    """Create a header provider that reads header values from volume-mounted files.

    This is used for auth tokens stored in Kubernetes Secrets that are mounted
    as volumes in the agent pod.  Kubernetes automatically updates mounted Secret
    volumes when the underlying Secret changes, so this provider always returns
    the current token without requiring a pod restart.

    Args:
        header_files: Mapping of header name to the absolute file path from
                      which the header value should be read.  The file content
                      is stripped of leading/trailing whitespace.

    Returns:
        A header provider function that reads from the given files on every call.
    """

    def provider(readonly_context: Optional[ReadonlyContext]) -> dict[str, str]:
        headers: dict[str, str] = {}
        for header_name, file_path in header_files.items():
            try:
                with open(file_path) as f:
                    headers[header_name] = f.read().strip()
            except OSError as e:
                logger.warning(
                    "Failed to read header file %s for header '%s': %s",
                    file_path,
                    header_name,
                    e,
                )
...
class HttpMcpServerConfig(BaseModel):
    ...
    # header_files maps header names to absolute file paths inside the agent pod.
    # The files are typically volume-mounted Kubernetes Secrets/ConfigMaps.
    # Values are read on every MCP request so token rotations take effect
    # automatically without a pod restart.
    header_files: dict[str, str] | None = None
...
class SseMcpServerConfig(BaseModel):
    ...
    allowed_headers: list[str] | None = None  # Headers to forward from A2A request to MCP calls
    # header_files maps header names to absolute file paths inside the agent pod.
    # See HttpMcpServerConfig.header_files for details.
    header_files: dict[str, str] | None = None
    ...
    def to_agent(self, name: str, sts_integration: Optional[ADKTokenPropagationPlugin] = None) -> Agent:
        ...
        if self.http_tools:
            for http_tool in self.http_tools:  # add http tools
                # Create a file-based header provider if header_files are configured.
                # This reads auth tokens from volume-mounted Kubernetes Secrets on every
                # MCP request so that token rotations are picked up without a pod restart.
                file_header_provider = None
                if http_tool.header_files:
                    file_header_provider = create_file_header_provider(http_tool.header_files)
                # Create header provider combining file-based, STS and allowed headers for this tool
                tool_header_provider = create_header_provider(
                    ...
                    base_header_provider=file_header_provider,
                )
                tools.append(
                    KAgentMcpToolset(
        ...
        if self.sse_tools:
            for sse_tool in self.sse_tools:  # add sse tools
                # Create a file-based header provider if header_files are configured.
                file_header_provider = None
                if sse_tool.header_files:
                    file_header_provider = create_file_header_provider(sse_tool.header_files)
                # Create header provider combining file-based, STS and allowed headers for this tool
                tool_header_provider = create_header_provider(
